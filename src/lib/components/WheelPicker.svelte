...
<div class="relative {className}" style="height: {ITEM_H * VISIBLE}px">
	<div
		class="pointer-events-none absolute inset-x-0 top-1/2 z-0 h-9 -translate-y-1/2 rounded-lg bg-[var(--shard-bg)]"
		aria-hidden="true"
	></div>
	<div
		...
		{#each items as item, i (item.value)}
			<div
				...
				class="flex cursor-pointer items-center justify-center tabular-nums transition-opacity duration-75
					{i === centerIndex
					? 'text-base font-semibold text-[var(--shard-text)]'
					: Math.abs(i - centerIndex) === 1
						? 'text-sm font-medium text-[var(--shard-text-muted)]'
						: 'text-sm text-[var(--shard-text-muted)] opacity-40'}"
