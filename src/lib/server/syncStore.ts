...
import { EMPTY_TAG_DIGEST, tagContribution, xorHex } from '$lib/syncHash';
 ...
 * `tag` is a client-computed keyed content hash; tags never leave the owner's handshake. */
...
export class SyncStore {
	...
	sync(
		...
		return this.database.transaction(() => {
			const account = this.database
				.prepare(
					`
				SELECT credential_hash, next_seq, envelope_count, ciphertext_bytes, wake_revision, tag_digest
				FROM accounts
				WHERE account_id = ?
			`
				)
			...
			let tagDigest = account.tag_digest ?? EMPTY_TAG_DIGEST;
			...
			const stageDeletion = this.database.prepare(`
				INSERT OR REPLACE INTO deleted_envelopes(account_id, slot, id, ciphertext, tag, deleted_at)
				SELECT account_id, slot, id, ciphertext, tag, ?
				FROM envelopes
			...
			const remove = this.database.prepare(`
				...
				WHERE account_id = ? AND slot = ? AND id = ?
				RETURNING length(ciphertext) AS ciphertext_bytes, tag
			`);
			...
			for (const deletion of deletions) {
				...
				const removed = remove.get(accountId, deletion.slot, deletion.id) as
					{ ciphertext_bytes: number; tag: string | null } | undefined;
				if (!removed) continue;
				envelopeCount -= 1;
				ciphertextBytes -= removed.ciphertext_bytes;
				if (removed.tag) tagDigest = xorHex(tagDigest, tagContribution(removed.tag));
			}
			...
			const upsert = this.database.prepare(`
				INSERT INTO envelopes(account_id, slot, seq, id, ciphertext, tag)
				VALUES (?, ?, ?, ?, ?, ?)
			...
			for (const upload of uploads) {
				...
				upsert.run(
					...
					upload.id,
					upload.ciphertext,
					typeof upload.tag === 'string' ? upload.tag : null
				);
				if (prior?.tag) tagDigest = xorHex(tagDigest, tagContribution(prior.tag));
				if (typeof upload.tag === 'string')
					tagDigest = xorHex(tagDigest, tagContribution(upload.tag));
			...
			if (added || deletions.length > 0) {
				this.database
					.prepare(
						`
					UPDATE accounts
					SET next_seq = ?, envelope_count = ?, ciphertext_bytes = ?, tag_digest = ?, updated_at = ?, last_seen_at = ?
					WHERE account_id = ?
				`
					)
					.run(sequence, envelopeCount, ciphertextBytes, tagDigest, seenAt, seenAt, accountId);
			} else {
	...
	/**
	 * All slots this account currently holds, with their content tags where
	...
	private ensureAccountColumns(): void {
		...
		if (!columns.has('tag_digest')) {
			this.database.exec(
				`ALTER TABLE accounts ADD COLUMN tag_digest TEXT NOT NULL DEFAULT '${EMPTY_TAG_DIGEST}'`
			);
		}
		for (const table of ['envelopes', 'deleted_envelopes']) {
			const envelopeColumns = new Set(
				(this.database.prepare(`PRAGMA table_info(${table})`).all() as Array<{ name: string }>).map(
					(column) => column.name
				)
			);
			if (!envelopeColumns.has('tag'))
				this.database.exec(`ALTER TABLE ${table} ADD COLUMN tag TEXT`);
		}
		this.database.exec(
